...
def calculate_pesq(args):
    ...
    #enh_speech_dir = os.path.join(workspace, "%ddb" % int(te_snr))
    names = os.listdir(enh_speech_dir)
    for (cnt, na) in enumerate(names):
        print(cnt, na)
        #dr1_felc0_si756_n38_3.wav
        na_list = na.split('_')
        print(na_list[-1][:-4], int(te_snr))
        if int(na_list[-1][:-4]) != int(te_snr):
            continue
